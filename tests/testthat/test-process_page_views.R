...
test_that("the object is a tibble", {
  expect_is(process_page_views(data.frame()), "tbl_df")
})
...
test_that("data can be read from a specific file", {
  expect_identical(process_page_views(filepath), processed)
})

test_that("data can be loaded from a course zip file", {
  expect_identical(process_page_views(zip_file), processed)
})
...
test_that("data can be processed from a specific class in a directory", {
  expect_identical(
    process_page_views(top_dir, class_id = "class_1"),
    processed
  )
})
